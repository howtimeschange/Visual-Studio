...
test('recoverJobs fails a stale running batch item without task id after its retry budget is exhausted', async () => {
...
test('recoverJobs requeues a stale running image task even after local retry budget is exhausted', async () => {
  const { mod, cleanup } = await importRunner()
  const sent = []
  const env = {
  ...
  try {
    const session = await mod.ensureSession(env, 'session_stale_task_recover', null)
    const job = await mod.createJob(env, {
      id: 'job_stale_task_recover',
      sessionId: session.id,
      userId: null,
      type: 'outfit_batch',
      status: 'running',
    ...
    await mod.createJobItems(env, job.id, [{
      ...
      outputJson: {
        imageTask: {
          id: 'task_stale_recoverable',
          status: 'running',
          poll_url: 'https://relay.example/v1/images/tasks/task_stale_recoverable',
        },
    ...
    assert.equal(nextItem.status, 'queued')
    assert.equal(nextItem.attemptCount, 3)
    assert.equal(nextItem.errorCode, null)
    assert.equal(nextItem.outputJson.imageTask.id, 'task_stale_recoverable')
