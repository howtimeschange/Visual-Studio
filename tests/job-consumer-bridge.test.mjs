...
test('queue consumer scheduled handler can preserve sub-five-second poll delay', async () => {
  const { mod, cleanup } = await importConsumer()
  const sent = []
  const options = []
  const env = {
  ...
  try {
    const session = await mod.ensureSession(env, 'session_consumer_scheduled_short_delay', null)
    const job = await mod.createJob(env, {
      id: 'job_consumer_scheduled_short_delay',
      sessionId: session.id,
      userId: null,
      type: 'outfit_batch',
      status: 'queued',
    ...
    await mod.createJobItems(env, job.id, [{
      ...
      outputJson: {
        imageTask: {
          id: 'task_short_watchdog',
          status: 'running',
          poll_url: 'https://relay.example/v1/images/tasks/task_short_watchdog',
        },
        imageTaskStatus: 'running',
    ...
    assert.equal(sent[0].jobId, job.id)
    assert.equal(sent[0].reason, 'recover')
    assert.equal(sent[0].delaySeconds, 3)
    assert.deepEqual(options[0], { delaySeconds: 3 })
  } finally {
    await cleanup()
  }
})
