...
import { resumeJob } from '../../../_lib/v2-runner'
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params, waitUntil }) => {
  try {
    ...
    return json(await resumeJob(env, jobId, waitUntil))
  } catch (error: any) {
    return json({ error: String(error?.message || 'Resume failed') }, error?.status || 502)
  }
}
